...
class FragmentCreatorReadGenerator(val environment: ProcessingEnvironment) {
    ...
    fun create(model: FragmentCreatorModel): TypeSpec.Builder {
        return TypeSpec.companionObjectBuilder()
                .addFun(createReadFun(model))
    }

    private fun createReadFun(model: FragmentCreatorModel): FunSpec {
        return FunSpec.builder("read")
                .addModifiers(KModifier.INTERNAL)
                .addParameter("fragment", ClassName.bestGuess("${model.packageName}.${model.originalClassName}"))
    ...
    private fun createParameterWithDefaultValueInitializeStatement(builder: FunSpec.Builder, args: VariableElement){
    ...
    private fun createGetParameterWithDefaultValueInitializeStatement(builder: FunSpec.Builder, param: VariableElement, type: TypeMirror){
        val key = param.simpleName.toString()
        when(type.toString()){
            "java.lang.Object" -> throw UnsupportedTypeException(param.asType().toString() + " is not supported on Bundle.")
            "java.lang.String" -> builder.addStatement("val %N = args.getString(%S, %S)", key, key, "")
            "boolean", "java.lang.Boolean" -> builder.addStatement("val %N = args.getBoolean(%S, %S)", key, key, false)
            "byte", "java.lang.Byte" -> builder.addStatement("val %N = args.getByte(%S, %L)", key, key, 0.toByte())
            "char", "java.lang.Character" -> builder.addStatement("val %N = args.getChar(%S, %L)", key, key, 0.toChar())
            "short", "java.lang.Short" -> builder.addStatement("val %N = args.getShort(%S, %L)", key, key, 0.toShort())
            "int", "java.lang.Integer" -> builder.addStatement("val %N = args.getInt(%S, %L)", key, key, 0)
            "long", "java.lang.Long" -> builder.addStatement("val %N = args.getLong(%S, %L)", key, key, 0L)
            "float", "java.lang.Float" -> builder.addStatement("val %N = args.getFloat(%S, %L)", key, key, 0f)
            "double", "java.lang.Double" -> builder.addStatement("val %N = args.getDouble(%S, %L)", key, key, 0.0)
            else -> {
                val typeElement = environment.typeUtils.asElement(type) as? TypeElement
                if (typeElement != null) {
                    createGetParameterWithDefaultValueInitializeStatement(builder, param, typeElement.superclass)
                }
            }
        }
    }
    ...
    private fun createParameterInitializeStatement(builder: FunSpec.Builder, args: VariableElement){
        ...
        if(extracted?.contains("%T") ?: false){
            if(holder.isEmpty()) {
                builder.addStatement(prefix + extracted, key, key, args.asType())
            }else{
                extracted = "%T().deserialize($extracted)"
                builder.addStatement(prefix + extracted, key, holder.serializer!!::class, key, holder.to!!::class)
            }
        }else{
            if (holder.isEmpty()) {
