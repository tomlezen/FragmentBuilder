...
class FragmentCreatorStep(val processingEnv: ProcessingEnvironment): BasicAnnotationProcessor.ProcessingStep{
        ...
        try {
            if(elementsByAnnotation != null){
                elementsByAnnotation[FragmentCreator::class.java]
                        ...
                        .forEach {
                            FragmentCreatorWriter(processingEnv, FragmentCreatorModel(it as TypeElement,  elementUtils)).write(getSourceLocation())
                        }
            }
        }catch (e: Exception){
