...
class FragmentCreatorBuilderGenerator(val environment: ProcessingEnvironment) {
    ...
    fun create(model: FragmentCreatorModel): TypeSpec {
        return TypeSpec.classBuilder("Builder")
                ...
                .addFun(createBuildFun(model.type, model.argsList))
                .build()
    }
    ...
    private fun createBuilderProperties(argsList: List<VariableElement>): List<PropertySpec> {
        ...
        argsList.mapTo(properties) {
            ...
            val propertySpecBuider = PropertySpec.varBuilder(it.simpleName.toString(), typeName, KModifier.PRIVATE)
    ...
    private fun createSetterFuns(model: FragmentCreatorModel, argsList: List<VariableElement>): List<FunSpec> {
    ...
    private fun generatePutMethodCall(buidler: FunSpec.Builder, args: VariableElement) {
        ...
        val format = extractPutMethod(holder.to ?: args.asType()) ?: throw UnsupportedTypeException(args.asType().toString() + " is not supported on Bundle.")
        ...
        if (holder.isEmpty()) {
            ...
            buidler.addStatement("$format%T().serialize(%N))", key, holder.serializer!!::class, args.simpleName)
        }
...
internal fun TypeMirror.asCustomTypeName(): TypeName{
    return this.accept(object : SimpleTypeVisitor7<TypeName, Void?>() {
        ...
        override fun visitDeclared(t: DeclaredType, p: Void?): TypeName {
            ...
            val enclosing = if (enclosingType.kind != TypeKind.NONE && !t.asElement().modifiers.contains(Modifier.STATIC)) enclosingType.accept(this, null) else null
            ...
            return (enclosing as? ParameterizedTypeName)?.nestedClass(rawType.simpleName(), typeArgumentNames) ?: constructor?.call(null, rawType, typeArgumentNames, false, listOf<AnnotationSpec>())!!
        }
